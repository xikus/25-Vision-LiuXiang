 ...
 */
template <typename _Tp, uint16_t StateDim, uint16_t MeasureDim>
class KalmanFilterX
{
    cv::Matx<_Tp, StateDim, StateDim> A;     //!< 状态转移矩阵 \f$A\f$
    cv::Matx<_Tp, StateDim, StateDim> At;    //!< 状态转移矩阵 `A` 的转置矩阵 \f$A^T\f$
    cv::Matx<_Tp, MeasureDim, StateDim> H;   //!< 观测转换矩阵 \f$H\f$
    cv::Matx<_Tp, StateDim, MeasureDim> Ht;  //!< 观测转换矩阵 `H` 的转置矩阵 \f$H^T\f$
    cv::Matx<_Tp, StateDim, 1> xhat;         //!< 后验状态估计 \f$\hat x\f$
    cv::Matx<_Tp, StateDim, 1> xhat_;        //!< 先验状态估计 \f$\hat x^-\f$
    cv::Matx<_Tp, StateDim, StateDim> Q;     //!< 过程噪声协方差矩阵 \f$Q\f$
    cv::Matx<_Tp, MeasureDim, MeasureDim> R; //!< 测量噪声协方差矩阵 \f$R\f$
    cv::Matx<_Tp, StateDim, StateDim> P;     //!< 后验状态误差协方差矩阵 \f$P\f$
    cv::Matx<_Tp, StateDim, StateDim> P_;    //!< 先验状态误差协方差矩阵 \f$P^-\f$
    cv::Matx<_Tp, StateDim, StateDim> I;     //!< 单位矩阵 \f$I\f$
    cv::Matx<_Tp, StateDim, MeasureDim> K;   //!< 卡尔曼增益 \f$K\f$
    cv::Matx<_Tp, MeasureDim, 1> z;          //!< 观测向量 \f$z\f$
...
public:
     ...
     */
    void init(const cv::Matx<_Tp, StateDim, 1> &state, _Tp error)
    {
        xhat_ = xhat = state;
        P_ = P = P.eye() * error;
    }
     ...
     */
    void init(const cv::Matx<_Tp, StateDim, 1> &state, const cv::Matx<_Tp, StateDim, 1> &error)
    {
        xhat_ = xhat = state;
        P_ = P = P.diag(error);
    }
     ...
     * @param[in] _A 状态转移矩阵
     */
    inline void setA(const cv::Matx<_Tp, StateDim, StateDim> &_A) { A = _A, At = _A.t(); }
     ...
     * @param[in] _H 观测转换矩阵
     */
    inline void setH(const cv::Matx<_Tp, MeasureDim, StateDim> &_H) { H = _H, Ht = _H.t(); }
     ...
     */
    inline void setR(const cv::Matx<_Tp, MeasureDim, MeasureDim> &measure_err) { R = measure_err; }
     ...
     */
    inline void setQ(const cv::Matx<_Tp, StateDim, StateDim> &process_err) { Q = process_err; }
     ...
     */
    inline void setP(const cv::Matx<_Tp, StateDim, StateDim> &state_err) { P_ = P = state_err; }
     ...
     * @return 后验状态估计
     */
    inline auto correct(const cv::Matx<_Tp, MeasureDim, 1> &measurement)
    {
        z = measurement;
