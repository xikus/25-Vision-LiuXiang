...
# 参数生成模块，包含以下主要功能：
#
#   1. srvl_generate_para:        根据给定目标及对应的参数规范文件 *.para 生成 C++ 文件
#   2. srvl_generate_module_para: 根据给定模块下的所有 para 目标生成 C++ 文件
#
# 以及以下次要功能：
#
...
function(type_correct value_type out_value_type)
    ...
    string(REGEX REPLACE "Matx" "cv::Matx" retval "${retval}")
...
function(para_parser file_name header_details source_details)
    ...
    foreach(substr ${out_val})
        ...
        if(type_sym MATCHES "uint" OR type_sym STREQUAL "size_t")
            set(ret_source "${ret_source}    int tmp_${id_sym} = 0;\n")
            set(ret_source "${ret_source}    para::readExcludeNone(fs[\"${id_sym}\"], tmp_${id_sym});\n")
...
#       srvl_generate_para(
#           mytarget        # 目标名称
#           MODULE mymodule # 模块名称为 combo
#       )
# ----------------------------------------------------------------------------
function(srvl_generate_para target_name)
    ...
    string(FIND "${SRVLPARA_${module_name}}" "${target_name}" target_idx)
    if(target_idx EQUAL -1)
        set(SRVLPARA_${module_name} "${SRVLPARA_${module_name}}" "${target_name}" CACHE INTERNAL "${module_name} parameters")
    endif()    
    # parser
    para_parser(${file_name} para_header_details para_source_details)
    set(template_path "${CMAKE_SOURCE_DIR}/cmake/paragen/runtime")
    set(para_include_path)
    # has module
    if(PARA_MODULE)
        set(para_include_path "srvlpara/${module_name}/${target_name}.h")
        configure_file(
    ...
    # dosen't have module
        ...
        set(para_include_path "srvlpara/${module_name}.hpp")
        configure_file(
            ${template_path}/para_generator_source.in
...
endfunction(srvl_generate_para target_name)

# ----------------------------------------------------------------------------
#   根据给定模块下所有的 para 目标，生成对应的 C++ 代码
#   用法:
#       srvl_generate_module_para(
#           <module_name>
#       )
#   示例:
#       srvl_generate_module_para(combo)
# ----------------------------------------------------------------------------
function(srvl_generate_module_para module_name)
    system_date(year month day)
    set(para_module_header_details "")
    foreach(_sub ${SRVLPARA_${module_name}})
        string(TOUPPER "${_sub}" upper)
        set(para_module_header_details "${para_module_header_details}\n#ifdef HAVE_SRVL_${upper}\n")
        set(para_module_header_details "${para_module_header_details}#include \"${module_name}/${_sub}.h\"\n")
        set(para_module_header_details "${para_module_header_details}#endif //! HAVE_SRVL_${upper}\n")
    endforeach()
    # generate C++ file
    set(template_path "${CMAKE_SOURCE_DIR}/cmake/paragen/runtime")
    configure_file(
        ${template_path}/para_generator_module.in
        ${CMAKE_CURRENT_LIST_DIR}/include/srvlpara/${module_name}.hpp
        @ONLY
    )
endfunction(srvl_generate_module_para module_name)
